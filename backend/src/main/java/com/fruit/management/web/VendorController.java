...
public class VendorController {
    ...
    public ApiResponse<VendorResponse> upsertProfile(
        ...
        return ApiResponse.ok(VendorResponse.from(vendorService.upsertCurrentVendor(openid, request)));
    }

    @GetMapping("/vendor/profile")
    public ApiResponse<VendorResponse> getCurrentProfile(HttpServletRequest httpRequest) {
        String openid = currentUserService.currentOpenid(httpRequest);
        return ApiResponse.ok(VendorResponse.from(vendorService.getCurrentVendor(openid)));
    }

    @GetMapping("/vendor/dashboard")
    public ApiResponse<DashboardResponse> getDashboard(HttpServletRequest httpRequest) {
        String openid = currentUserService.currentOpenid(httpRequest);
        return ApiResponse.ok(vendorService.getDashboard(openid));
    }
